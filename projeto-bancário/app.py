...
def register():
  ...
  Label(register_screen,
        text="Por favor, insira seus dados abaixo para se registrar",
        font=('Calibri', 12)).grid(row=0, sticky=N, pady=10)
  Label(register_screen, text="Name", font=('Calibri', 12)).grid(row=1,
                                                                 sticky=W)
  Label(register_screen, text="Age", font=('Calibri', 12)).grid(row=2,
                                                                sticky=W)
  Label(register_screen, text="Gender", font=('Calibri', 12)).grid(row=3,
                                                                   sticky=W)
  Label(register_screen, text="Pasword", font=('Calibri', 12)).grid(row=4,
                                                                    sticky=W)
  ...
  Entry(register_screen, textvariable=temp_password, show="+").grid(row=4,
                                                                    column=0)
  ...
  for name in all_accounts:
    if name == login_name:
      ...
      if login_password == password:
        ...
        Label(account_dashboard, text="Painel da Conta",
              font=('Calibri', 12)).grid(row=0, sticky=N, pady=10)
        Label(account_dashboard, text="Bem-vindo" + name,
              font=('Calibri', 12)).grid(row=1, sticky=N, pady=5)
        #Buttons
        Button(account_dashboard,
               text="Detalhes Pessoais",
               font=('Calibri', 12),
               width=30).grid(row=2, sticky=N, padx=10)
        Button(account_dashboard,
               text="Deposito",
               font=('Calibri', 12),
               width=30,
...
def deposit():
  ...
  #Label
  Label(deposit_screen, text="Deposito", font=('Calibri', 12)).grid(row=0,
                                                                    sticky=N,
                                                                    pady=10)
  current_balance_label = Label(deposit_screen,
                                text="Saldo Atual : R$" + details_balance,
                                font=('Calibri', 12))
  current_balance_label.grid(row=1, sticky=W)
  Label(deposit_screen, text="Valor :", font=('Calibri', 12)).grid(row=2,
                                                                   sticky=W)
...
def finish_deposit():
  ...
  current_balance_label.config(text="Saldo atual : R$" + str(update_balance),
                               fg="green")
...
def withdraw():
  ...
  #Label
  Label(withdraw_screen, text="Deposito", font=('Calibri', 12)).grid(row=0,
                                                                     sticky=N,
                                                                     pady=10)
  current_balance_label = Label(withdraw_screen,
                                text="Saldo Atual : R$" + details_balance,
                                font=('Calibri', 12))
  current_balance_label.grid(row=1, sticky=W)
  Label(withdraw_screen, text="Valor :", font=('Calibri', 12)).grid(row=2,
                                                                    sticky=W)
...
def personal_details():
  ...
  Label(personal_details_screen,
        text="Idade: " + details_age,
        font=('Calibri', 12)).grid(row=2, sticky=W)
  Label(personal_details_screen,
        text="Genero: " + details_gender,
        font=('Calibri', 12)).grid(row=3, sticky=W)
  Label(personal_details_screen,
        text="Balanço: " + details_balance,
...
def login():
  ...
  #Labels
  Label(login_screen, text="Faça login na sua conta",
        font=('Calibri', 12)).grid(row=0, sticky=N, pady=10)
  Label(login_screen, text='Nome de usuário',
        font=('Calibri', 12)).grid(row=1, sticky=W)
...
#Labels
Label(master, text="Banco Personalizado Beta",
      font=('Calibri', 14)).grid(row=0, sticky=N, pady=10)
Label(master,
      text="O banco mais seguro que você provavelmente já usou",
      font=('Calibri,12')).grid(row=1, sticky=N)
